...
class AttModel_mem4(CaptionModel):
    def __init__(self, opt):
        super(AttModel_mem4, self).__init__()
        ...
        self.ssg_attr_bbox_fc = nn.Sequential(nn.Linear(5, self.rnn_size),  #self.input_encoding_size + 5
                                nn.ReLU(inplace=True),
                                nn.Dropout(self.drop_prob_lm))

        self.img_v_feat_proj_fc = nn.Sequential(nn.Linear(256, self.rnn_size),
                                nn.ReLU(inplace=True),
                                nn.Dropout(self.drop_prob_lm)) #self.global_vis_features
    ...
    def ssg_graph_gfc(self, ssg_data):
        ...
        global_v_feats = ssg_data['global_v_feats'] #if self.global_vis_feature:
        ...
        if new_bbox_encoding: 
            ssg_bbox_feats = torch.zeros([ssg_bbox.size()[0], ssg_bbox.size()[1], self.rnn_size]).cuda()
            # ssg_bbox_masks_new = torch.zeros(ssg_obj.size()).cuda() ######
        ...
        for img_id in range(N_img):
            ...
            if new_bbox_encoding:
                obj_bbox_feats = self.ssg_attr_bbox_fc(ssg_bbox[img_id*seq_per_img, :N_obj].cuda().float())
            ...
            #kien
            box_feat = torch.zeros([N_obj, 5]).cuda()
            ...
            for obj_id in range(N_obj):
                ...
                if use_bbox_feature:
                    box_feat[obj_id] = ssg_bbox[img_id * seq_per_img, obj_id, :5].cuda().float() # check data type

                    if  not new_bbox_encoding:       
                        ### Subarna: add bbox features with the attributes
                        attr_feats_temp[obj_id] = attr_feats_temp[obj_id] +\
                                                    self.ssg_attr_bbox_fc(torch.cat((obj_feats_ori[obj_id], box_feat[obj_id])))  
                    else:
                        obj_bbox_feats[obj_id] = (obj_bbox_feats[obj_id] + obj_feats_ori[obj_id])/2.0
    ...
    def merge_ssg_att(self, ssg_data_new):
        """
        merge ssg_obj_feats, ssg_attr_feats, ssg_bbox_feats together
        <not using ssg_rela_feats, relations already integrated within ssg_obj_feats>
        :param ssg_data_new:
        :return: att_feats: [N_att, N_obj_max, rnn_size]
                 att_masks: [N_att, N_obj_max]
        """
        ...
        ## new bbox encoding
        if new_bbox_encoding:
            ssg_bbox_feats = ssg_data_new['ssg_bbox_feats'] 
    ...
    #     if new_bbox_encoding:
    #         ssg_bbox_feats = ssg_data_new['ssg_bbox_feats'] 
    #         # ssg_bbox_masks = ssg_data_new['ssg_bbox_masks']
    ...
    #             ### new bbox encoding
    #             N_bbox = N_obj
    #             N_att_max = max(N_att_max, N_rela + N_obj + N_attr + N_bbox)
    #         else:    
    ...
    #         if new_bbox_encoding:
    #             if N_obj != 0:
    #                 att_feats[img_id * seq_per_img:(img_id + 1) * seq_per_img, N_obj+N_attr+N_rela:N_obj+N_attr+N_rela+N_bbox, :] = \
    #                     ssg_bbox_feats[img_id * seq_per_img, :N_bbox, :] 
    #             att_masks[img_id * seq_per_img:(img_id + 1) * seq_per_img, :N_obj+N_rela+N_attr+N_bbox] = 1
...
class LSTM_mem4(AttModel_mem4):
    def __init__(self, opt):
        super(LSTM_mem4, self).__init__(opt)
        self.num_layers = 2
